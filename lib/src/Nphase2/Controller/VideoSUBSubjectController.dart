...
class VideoSubsubjectcontroller extends GetxController{
 ...
 FetchSubSubjectData()async{
  ...
  Map<String, String> queryParams = {
   ...
   "status" :catparentstatus.value==0?"":catparentstatus.value==1?"":catparentstatus.value.toString(),
   "pause_status": catparentstatus.value==1?"1":"0",

  };
  String queryString = Uri(queryParameters: queryParams).query;
  var child_video_categories_api = apiUrls().child_video_categories_api + '?' + queryString;

  log("FetchSubSubjectData url :${child_video_categories_api} ");
  try{
   var result = await apiCallingHelper().getAPICall(child_video_categories_api, true);
